...
%% Codifica??o dos coeficientes estoc?sticos

for k=1:M
    
    aprox_coef = DWT_coef(hbwt(k).Cw,hbwt(k).Lw,5);             % Extrai os coeficientes wavelet de aproxima??o do ?ltimo n?vel (conte?do estoc?stico)
    fas_estoc(k).aprox_coef = aprox_coef/rms(aprox_coef);       % Normaliza os coeficientes pela sua energia m?dia
    ...
    for n=1:wavelet_N
        %% Calcula e armazena a energia dos coeficientes de cada n?vel da transformada wavelet
    ...
    %% Realiza a regress?o linear da energia dos coeficientes estoc?sticos
...
%% Codifica??o dos coeficientes determin?sticos
for k=1:M/2
